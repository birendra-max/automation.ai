...
$result = $conn->query("SELECT COUNT(*) as total FROM call_schudle where status !='Completed'");
if (!$result) {
...
$stmt = $conn->prepare("SELECT id, phno, lab_name, status FROM call_schudle where status !='Completed' ORDER BY id ASC LIMIT ? OFFSET ?");
if ($stmt === false) {
